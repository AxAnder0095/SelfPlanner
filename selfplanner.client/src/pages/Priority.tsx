...
export const Priority = () => {
    const { priorityTasks } = useTestMocks();

    // Display types
    const BAR_DISPLAY: string = "priority-tasks-bar";
    const SQUARE_DISPLAY: string = "priority-tasks-square";
    const BAR_TASK: string = "priority-task-bar";
    const SQUARE_TASK: string = "priority-task-square";
    ...
    const displayMockTasks = () => {
        if (priorityTasks.length === 0) return <p>No tasks available.</p>;
        ...
        return (
            priorityTasks.map(task => (
                <div key={task.id} className={taskDisplayType}>
                    ...
                    <div className='priority-task-description'>
                        {/* <h3>Description</h3> */}
                        <p>{task.description}</p>
                    </div>
                    <div className='priority-task-status'>
                        {/* <h3>Status</h3> */}
    ...
    return (
        <div className="Priority">
            <section className="priority-header">
                <article className='priority-header-intro'>
                    <h1 className='priority-header-title'>Priority</h1>
                </article>
                <article className='priority-format-buttons'>
                    <button
                        type='button'
